...
const DashboardContent: React.FC<{ user: User }> = ({ user }) => {
  ...
  // View State for Student Management
  ...
  const fetchStudents = async () => {
    ...
    if (error) {
        ...
        const cleanData = rawData.map((s: any) => {
             ...
             if (!displayName) displayName = s.name; // Fallback

             return {
                ...s,
                name: displayName || 'Sem Nome',
                status: s.status || 'Activo',
                grade: s.grade || meta.grade || '10ª Classe',
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FA]">
      {/* Sidebar */}
      <div className="w-64 bg-white border-r border-slate-200 flex flex-col z-20 shadow-sm transition-all">
        ...
        <nav className="flex-1 p-4 space-y-1">
          <button onClick={() => { setActiveTab('students'); setViewMode('list'); }} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-all ${activeTab === 'students' ? 'bg-[#E3F2FD] text-[#137FEC] shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}>
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
         {/* Top Header - Only show if NOT in Detail/Edit View to save space/match flow */}
         {viewMode === 'list' && (
         ...
         {/* Content Area */}
         <div className={`flex-1 overflow-auto ${viewMode === 'list' ? 'p-8 max-w-[1600px] mx-auto w-full' : ''}`}>
                ...
                <>
                    ...
                    {viewMode === 'detail' && selectedStudent && (
                        <StudentDetailView 
                            ...
                            onEditStudent={() => setViewMode('edit')} 
                            onBack={() => setViewMode('list')} 
                        />
                    )}

                    {viewMode === 'edit' && selectedStudent && (
