...
const PaymentForm: React.FC<PaymentFormProps> = ({ onContinue, initialData }) => {
  ...
  const validatePhone = (method: PaymentMethodType, value: string) => {
    ...
    if (!cleanPhone) return t('required_field');
    if (!/^\d{9}$/.test(cleanPhone)) return t('phone_9_digits');
    
    if (method === 'MPESA') {
      if (!/^8[45]/.test(cleanPhone)) return t('mpesa_start_84_85');
    } else if (method === 'EMOLA') {
      if (!/^8[67]/.test(cleanPhone)) return t('emola_start_86_87');
    }
    return '';
  };
  ...
  const handleContinue = () => {
    if (!formData.paymentMethod) {
      setErrors({ paymentMethod: t('select_payment_method_error') });
      return;
    }
    ...
    if (!chargeFromState && (!formData.amount || parseFloat(formData.amount) <= 0)) {
        newErrors.amount = t('input_valid_amount');
    }
  ...
  return (
    <Layout title={t('make_payment') as any}>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-12 shadow-2xl shadow-slate-200 relative overflow-hidden">
            <div className="space-y-10">
              ...
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1">{t('choose_method')}</label>
              ...
              {formData.paymentMethod && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="space-y-2">
                     <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1">{t('amount')}</label>
                     <Input 
                       type="text" 
                       ...
                       error={errors.amount}
                     />
                     {chargeFromState && <p className="text-[10px] text-slate-400 font-bold px-1 italic">{t('payments')}: {chargeFromState.description}</p>}
                  </div>
                  ...
                  {isMobileMoney && (
                     <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                        <div className="space-y-2">
                          <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1">{t('phone_number')}</label>
                          <Input 
                            type="tel" 
                            placeholder="" 
                     ...
                     <div className="space-y-6 animate-in fade-in slide-in-from-top-2">
                        {!isGenerated ? (
                           <div className="py-2">
                              ...
                              <p className="text-[10px] text-center text-slate-400 mt-4 font-bold uppercase tracking-wider">{t('bank_notice')}</p>
                           </div>
                        ) : (
                           <div className="grid grid-cols-2 gap-4 animate-in zoom-in-95 duration-300">
                              <div className="p-5 rounded-[1.5rem] bg-blue-50/50 border-2 border-white shadow-inner flex flex-col items-center">
                                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('entity')}</p>
                                 <p className="text-2xl font-black text-slate-800 tracking-[0.2em]">{formData.entity}</p>
                              </div>
                              <div className="p-5 rounded-[1.5rem] bg-blue-50/50 border-2 border-white shadow-inner flex flex-col items-center">
                                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('reference')}</p>
              ...
              {!formData.paymentMethod && (
                <div className="py-12 flex flex-col items-center justify-center text-slate-300">
                   ...
                   <p className="font-black text-xs uppercase tracking-[0.2em]">{t('select_method')}</p>
                </div>
              )}
            </div>
          </div>
